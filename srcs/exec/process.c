...
/*   Updated: 2024/07/10 22:22:30 by bebrandt         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	parent_process(t_instruction *instruction)
{
	...
	{
		if (close_and_reset_pipe_fd(instruction, instruction->prev->fd) < 0)
			return (-1);
	}
	if (instruction->next != NULL)
	{
		if (close_and_reset_pipe_fd(instruction, instruction->fd + 1) < 0)
			return (-1);
	}
	return (0);
}
